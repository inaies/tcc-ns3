...
{
    LogComponentEnable("DdosBaseline", LOG_LEVEL_INFO);
    ...
    uint32_t nWifi = 173;
    bool tracing = true; // Mantém a geração de PCAPs
    ...
    if (tracing)
    {
        pointToPoint.EnablePcapAll("p2p-traffic-baseline");
        ...
        phy1.EnablePcap("ddos_baseline_ap1", apDevices1.Get(0)); 
        phy2.EnablePcap("ddos_baseline_ap2", apDevices2.Get(0)); 
        phy3.EnablePcap("ddos_baseline_ap3", apDevices3.Get(0)); 
    }
    
    NS_LOG_INFO("Iniciando Simulação Baseline...");
    Simulator::Stop(Seconds(601.0));
    Simulator::Run();
    
    // Exporta as métricas da rede sem defesa para um XML
    flowMonitor->SerializeToXmlFile("ddos-baseline-flowmon.xml", true, true);
