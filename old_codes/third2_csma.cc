...
main(int argc, char* argv[])
{
    ...
    for (uint32_t i = 0; i < csmaNodes1.GetN(); i++)
    {
        ...
        Ptr<Ipv6StaticRouting> sr = ipv6RoutingHelper.GetStaticRouting(ipv6);
        uint32_t ifSta = ipv6->GetInterfaceForDevice(csmaDevices1.Get(i));
        sr->SetDefaultRoute(csmaInterfaces1.GetAddress(0,1), ifSta);
    }
    ...
    Ptr<Ipv6> ipv6Ap1 = csmaDevices1.Get(0)->GetObject<Ipv6>();
    ...
    Ptr<Ipv6> ipv6Ap2 = csmaNodes.Get(0)->GetObject<Ipv6>();
    ...
    // // AP1 →rota para rede CSMA (2001:4::/64)
    // Ptr<Ipv6> ipv6Ap11 = wifiApNode.Get(0)->GetObject<Ipv6>();
    // uint32_t ifAp1ToAp3 = ipv6Ap11->GetInterfaceForDevice(ap1ap3.Get(0));
    // Ptr<Ipv6StaticRouting> srAp11 = ipv6RoutingHelper.GetStaticRouting(ipv6Ap1);
    // // srAp1->SetDefaultRoute(ap1ap2Interfaces.GetAddress(1,1), ifAp1ToAp2);
    // srAp11->AddNetworkRouteTo(Ipv6Address("2001:4::"), Ipv6Prefix(64),
    //                             ap1ap3Interfaces.GetAddress(1,1), 1);

    // // AP3 → rota de volta para a rede do STA via AP1
    // Ptr<Ipv6> ipv6Ap3 = p2pNodes.Get(2)->GetObject<Ipv6>();
    // uint32_t ifAp3ToAp1 = ipv6Ap11->GetInterfaceForDevice(ap1ap3.Get(1));
    // Ptr<Ipv6StaticRouting> srAp3 = ipv6RoutingHelper.GetStaticRouting(ipv6Ap2);
    // srAp3->AddNetworkRouteTo(Ipv6Address("2001:3::"), Ipv6Prefix(64),
    //                             ap1ap3Interfaces.GetAddress(0,1), 1);

    // *** PING IPv6 ***
    PingHelper ping(csmaInterfaces1.GetAddress(0, 1)); // endereço global do nó
...
// n5   n6   n7   n0 -------------- |
    ...
    if (tracing)
    {
        ...
        phy1.EnablePcap("third", csmaDevices1.Get(0));
